    ...
    const cards = [
        {
            ...
            route: "/diet-tracking/create-diet-plan",
        },
        {
        ...
        <section className="py-20 px-6 bg-white">
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                {cards.map((card, index) => (
                    <div
                        key={index}
                        className={`relative flex flex-col items-center ${card.offset ? "mt-[120px]" : ""
                            }`}
                    >
                        {/* Decorative Crown */}
                        {card.primary && (
                            <div className="flex gap-1 mb-2">
                                <div className="w-2 h-6 bg-[#134e27] rounded-full -rotate-12 translate-y-2" />
                                <div className="w-2 h-8 bg-[#134e27] rounded-full" />
                                <div className="w-2 h-6 bg-[#134e27] rounded-full rotate-12 translate-y-2" />
                            </div>
                        )}
                        ...
                        <div
                            className={`
                w-full rounded-[1000px] px-10 py-16 flex flex-col items-center
                max-w-[360px] text-center transition-transform hover:scale-103
                ${card.primary
                                    ? "bg-[#134e27] text-white"
                                    : "bg-[#f8f9f8] text-[#0a3d2e]"
                                }
              `}
                        >
                            {/* Icon */}
                ...
                `}
                            ...
                            {/* Content */}
                            <h3
                                className={`text-lg font-light font-unbounded mb-4 ${card.primary ? "text-white" : "text-[#134e27]"
                                    }`}
                            >
                                {card.title}
                            </h3>
                            ...
                            <div className="space-y-4 text-center">
                                <p
                                    className={`leading-snug font-light px-4 ${card.primary ? "text-[gray-100]" : "text-gray-800"
                                        }`}
                                >
                                    {card.desc}
                                </p>
                                ...
                                <div className="space-y-2 mb-10 w-full flex flex-col items-center px-6">
                                    {card.features.map((feat, i) => (
                                        ...
                                        >
                                            <span
                                                className={
                                                    card.primary ? "text-white" : "text-[#134e27]"
                                                }
                                            >
                                                {feat.icon}
                                            </span>
                            ...
                            >
                                <span
                                    className={`text-lg font-light font-unbounded ${card.primary ? "text-white" : "text-black"
                                        }`}
                                >
                                    {card.cta}
                                </span>
                                ...
                                <div
                                    className={`p-2 rounded-full ${card.primary
